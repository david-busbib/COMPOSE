...
#SBATCH --job-name=ablation_no_fusion
#SBATCH --output=logs/ablation_no_fusion_%j.log
#SBATCH --error=logs/ablation_no_fusion_err_%j.log

# Submit from the repo root: sbatch scripts/ablation_no_fusion.sh
...
# Ablation: w/o fusion — load paper-graph encoder checkpoint but skip cross-attention fusion
# between the two encoders; each encoder processes its modality independently.
RESUME="$CKPT_ROOT/encoder_only.pt"
TRAIN_CKPT_DIR="$CKPT_ROOT/ablation_no_fusion_${TIMESTAMP}"
